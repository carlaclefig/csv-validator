...
import { retryRecords } from "../services/api";
...
function ErrorTable({ errors, onErrorsUpdate }) {
  ...
  const currentErrors = editedErrors.slice(
    page * PAGE_SIZE,
    (page + 1) * PAGE_SIZE,
  );
  ...
  const handleRetry = async () => {
    const pageErrors = editedErrors.slice(
      page * PAGE_SIZE,
      (page + 1) * PAGE_SIZE,
    );
    const validToSend = pageErrors.filter(
      (e) => Object.keys(e.details).length === 0,
    );

    if (validToSend.length === 0) {
      alert("No hay registros válidos para reenviar en esta página");
      return;
    }

    setRetrying(true);
    try {
      const data = await retryRecords(validToSend);

      const sentRows = new Set(validToSend.map((e) => e.row));
      const remaining = editedErrors.filter((e) => !sentRows.has(e.row));

      setEditedErrors(remaining);
      onErrorsUpdate(remaining, data.data.success);

      if (remaining.length === 0) setPage(0);
      else if (page >= Math.ceil(remaining.length / PAGE_SIZE)) {
        setPage((p) => p - 1);
      }
    } catch (err) {
      alert(err.response?.data?.message || "Error al reenviar registros");
    } finally {
      setRetrying(false);
    }
  };
  ...
  return (
    <div className="space-y-3">
      ...
      {currentErrors.map((error, rowIndex) => (
        <div
          key={error.row}
          className="grid grid-cols-1 sm:grid-cols-12 gap-3 p-3 rounded-xl border border-slate-100 hover:border-slate-200 transition-colors"
        >
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-3 border-t border-slate-100">
        ...
        <button
          onClick={handleRetry}
          disabled={retrying}
          className="bg-slate-800 hover:bg-slate-900 disabled:bg-slate-400 text-white text-sm font-medium px-5 py-2 rounded-lg transition-colors w-full sm:w-auto"
        >
          {retrying ? "Enviando..." : "Reenviar"}
        </button>
      </div>
    </div>
  );
